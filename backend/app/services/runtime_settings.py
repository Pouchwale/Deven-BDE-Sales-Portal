...
def feedback_config(db: Session) -> dict[str, Any]:
    return {
        "scale_max": get(db, "feedback.rating_scale_max"),
        "threshold": get(db, "feedback.alert_threshold"),
        "min_responses": get(db, "feedback.alert_min_responses"),
        "window_days": get(db, "feedback.alert_window_days"),
        "expires_days": get(db, "feedback.request_expires_days"),
    }
